---
title: "My Projects"
draft: false
---

#

> Collection of my projects, both for career and fun (often overlapping with each other).

- [Theovim](#theovim)
- [Hunted Tiles](#hunted-tiles)
- [My Dotfiles](#my-dotfiles)
- [i3 & Sway Tips](#i3--sway-tips)
- [Clean Linear Algebra Homework Template](#clean-linear-algebra-homework-template)
- [vimrc Guide](#vimrc-guides)
- [CS240 Comprehensive Review](#CS240-Comprehensive-Review)

## Theovim

[*GitHub Repository ⧉*](https://github.com/theopn/theovim)

Theovim is a Neovim IDE configuration aimed toward CS majors. More description coming soon...

## Hunted Tiles

[*GitHub Repository ⧉*](https://github.com/theopn/hunted-tiles)

Hunted Tiles is a collection of tiling Wayland compositor configurations along with other scripts to provide a DE-like Wayland experience. Currently, Sway and Hyprland (work in progress) are supported. More description coming soon...

{{< figure src="/img/projects/2022-09-05_Hunted-Tiles_Screenshot.png" alt="Hunted Tiles screenshot" position="center" >}}

## My Dotfiles

[*GitHub Repository ⧉*](https://github.com/theopn/dotfiles)

Well, does this count as a project? But this is the repository I spend most of my time committing for sure. This repository includes configurations for text editors like Vim, Neovim, and Emacs, zsh, kitty, tmux, and other terminal/shell related programs, i3 & Polybar and Sway & Waybar, and weirdos like Mutt and w3m.

{{< figure src="/img/projects/2022-07-25_Fedora_Screenshot.png" alt="My i3 setup" position="center" caption="My i3 setup" captionPosition="center" captionStyle="color: black;" >}}

## i3 & Sway Tips

[*GitHub Repository ⧉*](https://github.com/theopn/i3-sway-tips)

Collection of commands, keybindings, and recommended applications for i3 or Sway window manager beginners. Some of the recommendations are subjective and does not contain all the information you might need, but I would love to make this a community project so feel free to suggest or correct information through issues or to me personally!

## Pytranscript

[*GitHub Repository ⧉*](https://github.com/theopn/pytranscript)

This A-worthy-cs-freshman-level Python project is a simple CLI GPA calculator I use when I cannot wait a week for my professors to finalize my grades.

## Clean Linear Algebra Homework Template

[*GitHub Repository ⧉*](https://github.com/theopn/clean-la-hw-template)

Linear algebra is fun, but it can be more fun with the right LaTeX template.

## vimrc Guides

[*Part 1 ⧉*]({{< ref "/writing/2022-06-20_vimrc_guide" >}} "vimrc Guide Part 1")

Guides to create a personal Vim environment and boosting your productivity. Right here on this website.

## CS240 Comprehensive Review

[*GitHub Repository ⧉*](https://github.com/theopn/cs240-comprehensive-review)

17 pages of C.

{{< figure src="/img/projects/2022-05-03_CS240Review_joke.jpg" alt="Joke of the century" position="center" >}}

